...
$headers = @{
  ...
  "User-Agent" = "Japanese-Study-Hub-Tunnel-Installer"
}
...
if ((Test-Path -LiteralPath $destination) -and -not $Force) {
  ...
  if ($installedVersion -match [regex]::Escape($release.tag_name.TrimStart("v"))) {
    [pscustomobject]@{
      ...
      reason = "requested version already installed"
...
$tempDir = Join-Path $tempRoot ("jstudy-tunnel-" + [guid]::NewGuid().ToString("N"))
...
try {
  ...
  $checksumLine = Get-Content -LiteralPath $checksumsPath | Where-Object {
    $_ -match ([regex]::Escape($asset[0].name) + "$")
  } | Select-Object -First 1
  if (-not $checksumLine) {
    throw "The release checksum file does not contain the selected Windows asset."
  }
  ...
  if ($executables.Count -ne 1) {
    throw "Expected one tunnel-client.exe, found $($executables.Count)."
  }
  ...
  [pscustomobject]@{
    ...
    executableSha256 = (Get-FileHash -Algorithm SHA256 -LiteralPath $destination).Hash
  } | ConvertTo-Json -Depth 4
}
finally {
