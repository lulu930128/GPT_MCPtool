param(
    [ValidateSet("copy_mcp_url", "copy_health_url", "copy_tunnel_id", "open_mcp_health", "open_tunnel_ui", "open_runtime_logs")]
    [string]$Action,
    [switch]$SelfTest
)
...
$supportedActions = @("copy_mcp_url", "copy_health_url", "copy_tunnel_id", "open_mcp_health", "open_tunnel_ui", "open_runtime_logs")
...
function Show-Message([string]$Message, [Windows.Forms.MessageBoxIcon]$Icon) {
    [Windows.Forms.MessageBox]::Show($Message, "Project Reading MCP", 'OK', $Icon) | Out-Null
}
...
try {
    ...
    Show-Message "The requested action could not be completed. Check the Project Reading runtime settings." ([Windows.Forms.MessageBoxIcon]::Warning)
