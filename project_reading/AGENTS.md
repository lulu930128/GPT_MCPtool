...
- File return tools remain read-only only when they reuse the shared path guard, create no local
  files, and preserve the existing deny policy and bounded output limits.
