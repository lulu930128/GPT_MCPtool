...
const expectedTools = [
  ...
  "english_search_items", "english_set_manual_labels",
].sort();

test("MCP publishes the independent strict 12-tool contract", async () => {
  const handle = await startEnglishStudyHttpServer({ hubBaseUrl: "http://127.0.0.1:1", hubTimeoutMs: 2000, host: "127.0.0.1", port: 0 });
  const client = new Client({ name: "contract-test", version: "0.2.0" });
  ...
  try {
    ...
    assert.equal(health.toolCount, 12);
...
test("representative output validates and Hub errors stay typed", async () => {
  ...
  const client = new Client({ name: "projection-test", version: "0.2.0" });
  const transport = new StreamableHTTPClientTransport(new URL(handle.url));
  try {
    await client.connect(transport);
