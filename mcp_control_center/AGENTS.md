...
本目錄是七個已登錄本機 MCP runtime 的 Windows orchestration 與 observability layer，不是 MCP server，也不擁有任何 domain data。
...
- `scripts\Restart-Tray.cmd` 只可 exact-path replacement 中樞 tray，不停止任何 component。
