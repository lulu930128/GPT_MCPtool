...
Assert-Equal $manifest.schemaVersion 1 "current live manifest remains schema version 1"
Assert-True (@($manifest.components | Where-Object { -not $_.autoStart }).Count -eq 0) "all unified components are auto-start enabled"
Assert-True (@($manifest.components | Where-Object { $_.runtimeMode -ne "legacy-tray" }).Count -eq 0) "schema v1 components normalize to legacy tray mode"
$legacyStartPlan = Invoke-McpCcComponentAction -Manifest $manifest -ComponentId "project_reading" -Action ensure_running -PlanOnly
Assert-Equal $legacyStartPlan.manifestAction "start" "legacy ensure_running maps to the existing start action"
$legacyReloadPlan = Invoke-McpCcComponentAction -Manifest $manifest -ComponentId "project_reading" -Action reload_runtime -PlanOnly
Assert-Equal $legacyReloadPlan.manifestAction "restart" "legacy reload_runtime maps to the existing restart action"
Assert-Throws { Invoke-McpCcComponentAction -Manifest $manifest -ComponentId "project_reading" -Action restart_core -PlanOnly } "legacy tray does not claim the v3 restart_core capability"
...
Assert-True (Test-McpCcCommandLineContains -CommandLine 'python.exe -m uvicorn memory_core.main:app --port 8765' -ExpectedFragments @("memory_core.main:app")) "listener module identity is accepted without a repo path"
Assert-True (-not (Test-McpCcCommandLineContains -CommandLine 'python.exe -m uvicorn other.main:app --port 8765' -ExpectedFragments @("memory_core.main:app"))) "wrong listener module is rejected"
...
Assert-Equal $blocked "BlockedUpstream" "dependency failure is explicit"
...
try {
    ...
    $controllerSource = @'
param(
    [ValidateSet("SelfTest", "EnsureRunning", "RestartCore", "ReloadRuntime")]
...
if ($Action -eq "SelfTest") {
    [pscustomobject]@{
        ...
        capabilities = @("ensure_running", "restart_core", "reload_runtime", "show_diagnostic_tray")
...
'@
    ...
    Assert-True (@($v2Manifest.components[0].capabilities).Count -eq 4) "controller capabilities are normalized"
