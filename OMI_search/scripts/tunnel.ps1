...
if ([string]::IsNullOrWhiteSpace($TunnelClientPath)) {
  $TunnelClientPath = "C:\GPT_MCPtool\project_reading\vendor\tunnel-client\tunnel-client.exe"
}
...
switch ($Action) {
  ...
  "Init" {
    if ([string]::IsNullOrWhiteSpace($TunnelId)) {
      throw "TunnelId is required for Init. Create or choose a tunnel in ChatGPT/OpenAI first, then rerun with -TunnelId tunnel_..."
    }
    Invoke-TunnelClient @(
      "init",
