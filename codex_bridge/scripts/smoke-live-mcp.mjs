...
try {
  ...
  const resource = await client.readResource({ uri: "ui://codex-bridge/chat-workspace-v11.html" });
  const widgetHtml = resource.contents.find((content) => content.uri === "ui://codex-bridge/chat-workspace-v11.html")?.text;
  ...
  assert.match(widgetHtml, /codex_local_thread_list/);
